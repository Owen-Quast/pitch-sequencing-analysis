...
Baseline whiff rate is computed per pitch type from mid-sequence pitches only (first and last pitches of each at-bat excluded), keeping the baseline pool consistent with the sequence whiff rates. Positive Setup Value means a sequence produces more whiffs than that pitch earns on average; negative means it underperforms.
The Setup Value color scale is set asymmetrically (−0.04 to +0.08) to fully display the SI → CH finding without clipping.
